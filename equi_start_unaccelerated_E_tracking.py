...
if len(sys.argv) != 3:
    print("Usage: python Fourier... num_trajectories dt")
    exit()
...
N_equi_trajs = 200 # number of trajectories (equivalent to number of measurements of observables)
sweep = 0
...
def HMC_E_track(q_current, delta_t, lf_steps,sweep):
    ...
    H_traj_outfile.write("{}\n".format(H))
    U_proposed = S(q)
    K_proposed = (1/2.0)*(np.sum(p**2)) 

    H_proposed = U_proposed + K_proposed
    
    delta_H = H_proposed - H_current 
    delta_H_outfile.write("{}\n".format(delta_H)) # write out values of delta_H to check for correct behaviour of HMC 
...
outfile =           open("per_line_equi_start_unaccelerated_1d_field_energy_tracking.dat".format(sys.argv[1]),"w")
s_traj_outfile =    open("per_line_equi_start_ordered_var_t_s_traj_outfile_energy_tracking.dat", "w")
delta_H_outfile =   open("per_line_equi_start_delta_h_energy_tracking.dat","w")
H_traj_outfile =    open("per_line_equi_start_ordered_var_t_h_unaccel_trajenergy_tracking.dat", "w")



for equiSampl in range(100): # sample a field distribution from equilibrated system 100 times
    print("equiSampl == {}\n".format(equiSampl))
