...
N_tau = 11 # size of the lattice (i.e., the length of the harmonic oscillator's path in time)
N_trajs = 200 # number of trajectories (equivalent to number of measurements of observables)
...
m = 0.1 # mass of particle. Do an array divided by array element-wise if you want multiple particles with different masses.
m_eff = m
...
def HMC(q_current, delta_t, lf_steps,sweep):
    ...
    for l in range(1, int((N_tau+1)/2)):
        print(l)
        cov = np.sqrt(1.0*(m**2 + 4*(np.sin(np.pi*l/N_tau))**2))
        p_t_r = (1/np.sqrt(2.0))*np.random.normal(0, np.sqrt(N_tau)*cov)
        p_t_i = (1/np.sqrt(2.0))*np.random.normal(0, np.sqrt(N_tau)*cov)
    ...
    if sweep == 100 or sweep == 101 or sweep == 102:
    ...
    # if sweep > 0:
        #calculate av_sum_q_sq
        # av_sum_q_sq = np.mean(np.sum(q)**2)
        # av_sum_phi_sq_outfile.write("{}\n".format(av_sum_q_sq))
        # av_x_outfile.write("{}\n".format(np.mean(q_current)))
        # av_x_sq_outfile.write("{}\n".format(np.mean(q_current)**2))
        # x_sq_file.write("{}\n".format(np.sum(q_current**2)))
...
# av_sum_phi_sq_outfile = open("m_0.01_equilib_av_sum_phi_sq_outfile_FA.txt", "w")
# delta_H_outfile = open("equilib_hmc_delta_H_new_sampling.txt","w")
outfile         = open("fa_path_vis.txt","w")
# s_outfile       = open("equilib_fourier_acc_s_1d_scalar_field_new_sampling.txt", "w")
# av_x_sq_outfile         = open("equilib_FA_HO_mag_av_x_sq_outfile.txt", "w")
# av_x_outfile            = open("equilib_FFT_FA_mag_av_x_outfile.txt", "w")
# x_sq_file = open("m_0.01_equilib_x_sq_file_FA.txt", "w")
