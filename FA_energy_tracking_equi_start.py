...
if len(sys.argv) != 4:
    print("Usage: python Fourier... lf_steps m_eff delta_t")
    exit()
...
lf_steps = int(sys.argv[1])  # leapfrog steps in a trajectory. say 10000 steps. 0.01 stepsize
#lf_steps = 10000
...
def HMC(q_current, delta_t, lf_steps,sweep):
    ...
    for i in range(lf_steps):
        ...
        #q_corr = q + delta_t*(M_inv.dot(p))
        q = q + delta_t*np.real(np.fft.ifft(M_k_inv.dot(p_k))) # am I making best use of the fact M_k is diagonal here?
...
s_outfile       = open("equi_start_500equis_m_eff_{}_dt_{}_lf_steps_{}_av_track_fourier_acc_s_1d_scalar_field_mod_sampling.txt".format(m_eff, delta_t, lf_steps), "w")
delta_H_outfile = open("equi_start_500equis_m_eff_{}_dt_{}_lf_steps_{}_av_hmc_delta_H_mod_sampling".format(m_eff, delta_t, lf_steps),"w")
H_traj_outfile  = open("equi_start_500equis_m_eff_{}_dt_{}_lf_steps_{}_av_FFT_accelerated_h__trajenergy_tracking.dat".format(m_eff, delta_t, lf_steps), "w")

for equiSampl in range(500): # sample a field distribution from equilibrated system 100 times
    print("equiSampl=={}".format(equiSampl))
    for sweep in range(N_equi_trajs): # use 200 trajectories to equilibrate
        q_current,p_current,accrate = HMC(q_current, delta_t_equi, lf_steps_equi,sweep)
    equi_q_current = q_current # 1 sample of an equilibrated field config. Store and now run single equilibrated trajectory with 100 diff init momenta
    for mom_sampl in range(100):
    # the current plotting code I have deals with paths, not averaged paths
    #U = S(q_current)
    #grad_S = grad_S(q_current)
        HMC_E_track(equi_q_current, delta_t, lf_steps,sweep) # track S vs t and H vs t 
