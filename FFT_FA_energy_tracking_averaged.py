...
s_outfile       = open("m_eff_{}_dt_{}_lf_steps_{}_av_track_fourier_acc_s_1d_scalar_field_mod_sampling.txt".format(m_eff, delta_t, lf_steps), "w")
delta_H_outfile = open("m_eff_{}_dt_{}_lf_steps_{}_av_hmc_delta_H_mod_sampling".format(m_eff, delta_t, lf_steps),"w")
H_traj_outfile  = open("m_eff_{}_dt_{}_lf_steps_{}_av_FFT_accelerated_t_h_unaccel_trajenergy_tracking.dat".format(m_eff, delta_t, lf_steps), "w")

# run 1 trajectory 100 times with different initial momenta
for sweep in range(100):
    # the current plotting code I have deals with paths, not averaged paths
    #U = S(q_current)
    #grad_S = grad_S(q_current)
    HMC(q_current, delta_t, lf_steps,sweep)
