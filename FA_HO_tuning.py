...
if len(sys.argv) != 3:
    print("Usage: python Fourier... outfile_name m_eff dt")
    exit()
...
lf_steps = 10000  # leapfrog steps in a trajectory. Keep number of steps fixed at 10000 
delta_t = float(sys.argv[3])  # leapfrog timestep in computer time. Passed as cmd line arg for use in bash
...
outfile         = open("dt_{}_m_{}_tuning_fourier_accelerated_1d_scalar_field_mod_sampling.txt".format(sys.argv[1], sys.argv[2]),"w") # pass desired M and stepsizec from the command line
s_outfile       = open("dt_{}_m_{}_tuning_fourier_acc_s_1d_scalar_field_mod_sampling.txt".format(sys.argv[1], sys.argv[2]), "w")
delta_H_outfile = open("dt_{}_m_{}_hmc_delta_H_mod_sampling".format(sys.argv[1], sys.argv[2])),"w")
