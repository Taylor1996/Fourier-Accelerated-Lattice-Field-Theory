...
s_outfile       = open("small_lambda_equi_start_m_eff_{}_dt_{}_lf_steps_{}_av_s_track_FA_AHO.txt".format(m_eff, delta_t, lf_steps), "w")
delta_H_outfile = open("small_lambda_equi_start_m_eff_{}_dt_{}_lf_steps_{}_delta_H_FA_AHO.txt".format(m_eff, delta_t, lf_steps),"w")
H_traj_outfile  = open("small_lambda_equi_start_m_eff_{}_dt_{}_lf_steps_{}_h_tracking_FA_AHO.txt".format(m_eff, delta_t, lf_steps), "w")
...
for equiSampl in range(100): # sample a field distribution from equilibrated system 100 times
    ...
    #grad_S = grad_S(q_current)
        HMC_E_track(equi_q_current, delta_t, lf_steps,mom_sampl) # track S vs t and H vs t 
