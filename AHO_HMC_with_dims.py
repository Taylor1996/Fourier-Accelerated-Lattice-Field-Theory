...
def S(q):
    ...
    q_f = q**2 - f_sqr*np.ones(N_tau)
    S =  S = (1/(2.0*a))*m*sqr_term +  a*Lambda*np.sum(q_f**2)  
...
def grad_S(q):
    ...
    q_f = q**2 - f_sqr*np.ones(N_tau)
    grad_S = (m/a)*(-q_forward + 2*q - q_backward) + 4*a*Lambda*q*(q_f)
...
outfile = open("correct_mins_{}_{}_{}.txt".format(N_tau, a, N_trajs),"w")
